            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <span
                            class="border-t border-b border-l border-brand-light px-3 py-2 bg-blue-400 text-white no-underline">{{ $page }}</span>
                        {{-- Agrega la clase bg-blue-500 y text-white para cambiar el color de fondo y texto a azul y blanco respectivamente --}}
                    @else
